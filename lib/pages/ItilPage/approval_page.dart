...
class ITILApprovalPage extends StatefulWidget {
  ITILApprovalPage({Key key, this.title}) : super(key: key);

  final String title;

  @override
  _ITILApprovalPageState createState() => new _ITILApprovalPageState();
}

class _ITILApprovalPageState extends State<ITILApprovalPage> {
  ...
  void initState() {
    super.initState();
    approvalList = _getApprovalList(1, pageSize);
  }
  ...
  void _onRefresh(dynamic controller) {
    new Future.delayed(const Duration(milliseconds: 200)).then((val) {
      currentPage = 1;
      setState(() {
        approvalList = _getApprovalList(currentPage, pageSize);
      });
  ...
  void _onLoad(dynamic controller) {
    new Future.delayed(const Duration(milliseconds: 200)).then((val) {
      currentPage++;
      setState(() {
        approvalList.addAll(_getApprovalList(currentPage, pageSize));
      });
  ...
  List<Map<String, dynamic>> _getApprovalList(int currentPage, int pageSize) {
    ...
    return PullPushList(
      ...
      child: ListView.builder(
        itemBuilder: (BuildContext context, int index) {
          return _listCard(context, approvalList[index], index);
        },
        physics: BouncingScrollPhysics(),
        itemCount: approvalList.length ?? 0,
      ),
    );
  }
